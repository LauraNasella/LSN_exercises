...
double Individuo::L1() const{
	double lunghezza=0.;
 	int n = this->percorso.size()-1;
  	for (int i=0; i<n; i++){
    		lunghezza = lunghezza + distanza2(i,i+1);
  	}
	...
	lunghezza = lunghezza + d_partenza_0 + d_fine_partenza;
	return lunghezza;
}
...
Individuo& Individuo::operator=(const Individuo& ind){
	...
	percorso = ind.percorso;
	path_totale = ind.path_totale;
	
	partenza = ind.partenza;
...
Popolazione::Popolazione(int number, Random & rnd, Individuo primo) : N_pop(number) {
	if(primo.check() == false){ //false per me vuol dire che non passa nella stessa città più volte
