...
//1)Campionamento discreto
	...
	x = new int[N];
    	...
    	for(int i=0; i<N; i++){
        	...
        	for(int j=0; j<L; j++){
        		double ST = S0;
        		for(int k=0; k<step_temp; k++){
            			double z = rnd.Gauss(0,1);
            			ST = ST * exp((r - 0.5 * pow(sigma,2)) * deltat + sigma * z * sqrt(deltat));
            		}
	...
	file_out.open("Call_discreto.out");
   	file_out1.open("Put_discreto.out");	
